...
function load(list_cursor, add_playlist, target_time)
	...
	if file_exists(list_contents[#list_contents - list_cursor + 1].found_path) or starts_protocol(protocols, list_contents[#list_contents - list_cursor + 1].found_path) then
		...
		if o.osd_messages == true then
			mp.osd_message('⚠️  Diese Datei existiert nicht mehr, du Stinker.\nDer Pfad hat sich geändert, oder du hast die NAS nicht gemounted \n(Diese Meldung geht in 15 Sekunden weg).\n\n\n\nFile not found: ' .. list_contents[#list_contents - list_cursor + 1].found_path, 15)
		end
...
mp.register_event('file-loaded', function()
	list_close_and_trash_collection()
	filePath, fileTitle, fileLength = get_file()
	loadTriggered = false --1.1.5# for resume and resume-notime startup behavior (so that it only triggers if started as idle and only once)
	if (resume_selected == false and seekTime > 0) then
		mp.commandv('seek', seekTime, 'absolute', 'exact')
